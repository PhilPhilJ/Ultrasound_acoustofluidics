...
max_fit_2=round(float(xdata[ydata==max(ydata)]),6)
...
freq_near_fwhm_2 = xdata[np.isclose(func2(xdata, *popt[3:]), max(func2(xdata, *popt[3:]))/2, atol=0.002, rtol=0)]
if len(freq_near_fwhm_2) % 2 == 0:
    f_lower_2 = np.mean(freq_near_fwhm_2[0:int(len(freq_near_fwhm_2)/2+1)])
    f_upper_2 = np.mean(freq_near_fwhm_2[int(len(freq_near_fwhm_2)/2+1):len(freq_near_fwhm_2)])
elif len(freq_near_fwhm_2) % 2 != 0:
    raise(ValueError)
FWHM_fit_2 = f_upper_2-f_lower_2
gain_2 = max_fit/FWHM_fit_2

fig, ax = plt.subplots(figsize=(10,7.5))
plt.fill_between(xdata, ydata_lower, ydata_upper, color='grey', alpha=0.3, interpolate=True, label='_nolabel_')
ax.plot(freq, reciprocal_t_star,'ro', markersize='10', label='Data')
...
ax.plot(xdata, ydata, linestyle='solid', color ='black', label='Two peak fit')
ax.plot(xdata, func2(xdata, *popt[0:3]), linestyle='dashed', color ='black', label='One peak fit')
ax.plot(xdata, func2(xdata, *popt[3:]), linestyle='dashed', color ='black', label='_nolabel_')
ax.hlines(y=max(func2(xdata, *popt[3:]))/2, xmin=1.901150150150149987, xmax=1.902152152152152054, color='black')
ax.hlines(y=max(func2(xdata, *popt[:3]))/2, xmin=1.895814814814814842, xmax=1.900109109109109, color='black')
ax.vlines(x=1.89795918, ymin=0, ymax=max(func2(xdata, *popt[0:3])), color='black', linestyle='dotted')
ax.vlines(x=1.90164995, ymin=0, ymax=max(func2(xdata, *popt[3:])), color='black', linestyle='dotted')
...
#%%
plt.savefig('/Users/joakimpihl/Desktop/Resonance.png', dpi=300, bbox_inches='tight')
