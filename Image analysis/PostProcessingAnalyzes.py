...
#vid_num = 8
for vid_num in range(0,100):
    plt.close('all')
    path = '/Users/joakimpihl/Desktop/DTU/7. Semester/Bachelorprojekt/Results/Biiiig sweep/I_norm/I_norm run 1'+str(vid_num)+'.csv'
    path_out = '/Users/joakimpihl/Desktop/DTU/7. Semester/Bachelorprojekt/Results/Biiiig sweep/1'
    ...
    t_index_0 = np.argmin(np.abs(timestamps))
    ...
    popt, pcov = curve_fit(func, timestamps[t_index_0:len(timestamps)], intensities[t_index_0:len(timestamps)]) #Fitting 
    ...
    plt.savefig(path_out + 'Plots_diff_fit/run 1'+str(vid_num)+' MHz.png', dpi=300, bbox_inches='tight')
        
    df = pd.DataFrame({'Run':[vid_num], 't_star':[t_star], 'alpha':[alpha], 'R (Fit)':[R_fit], 'R squared':[r_squared]})
    
    if os.path.exists(path_out + 'Resonance freq_diff_f1it.csv') != True:
        df.to_csv(path_out + 'Resonance freq_diff_fit1.csv', sep=',')
    else:
        df.to_csv(path_out + 'Resonance freq_diff_fit1.csv', sep=',', header=False, mode='a')