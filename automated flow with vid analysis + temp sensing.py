...
def speed_of_sound(T):
    # Temperature has unit of Kelvin
    # Speed of sound has unit of m/s
    # Define the coefficients for Cs - DOI:10.1103/PhysRevE.90.043016
    c0 = -3.1420310908*10**4
    c1 = 4.4207693183*10**2
    c2 = -2.4555531077*10**0
    c3 = 7.0411177557*10**(-3)
    c4 = -1.0353739103*10**(-5)
    c5 = 6.1949139692*10**(-9)

    # Find the speed of sound at the given temperature
    cs = c0 + c1*T + c2*T**2 + c3*T**3 + c4*T**4 + c5*T**5
...
temperature_array = np.array([]) # array to store the temperature values
len_temp_array = np.array([]) # array to store the length of the temperature array
frequency0 = 1.898 # frequency in MHz  
runs = 2
...
for i in range(runs):       
    ...
    while camera.IsGrabbing():
        ...
        if grabResult.GrabSucceeded():
            ...
            temperature_array = np.append(temperature_array, find_temp()) # Appends the temperature to the temperature array
